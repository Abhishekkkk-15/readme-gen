...
export class LocalAnalyzerService {
  ...
  public async analyze(
    ...
    const metadataFiles = allFilePaths.filter(
      (f) =>
        f === "README.md" ||
        f === "README.mdx" ||
        f === "README.txt" ||
    ...
    for (const file of metadataFiles) {
      ...
      fileContents[file] = fs.readFileSync(
    ...
    const importantFiles = Array.from(
      ...
      .filter((f) => fs.existsSync(path.join(this.rootPath, f)))
      .slice(0, 70);

    console.log(
      `[LocalAnalyzer] Total files prioritized for analysis: ${importantFiles.length} (including ${manualImportantFiles.length} manual files)`
    );
    const sourceFiles = importantFiles.filter((f) =>
      f.match(/\.(ts|js|tsx|jsx|py|go)$/)
    );
    console.log(
      `[LocalAnalyzer] Found ${sourceFiles.length} source files for definition extraction.`
    );
    ...
    for (const filePath of importantFiles) {
      if (
        !importantContents[filePath] &&
        fs.existsSync(path.join(this.rootPath, filePath))
      ) {
        importantContents[filePath] = fs.readFileSync(
    ...
    console.log(
      `[LocalAnalyzer] Extracted ${totalSnippetCount} definitions from ${extractedFileCount} files.`
    );
    ...
    const existingReadmePath = ["README.md", "README.mdx", "README.txt"].find(
      (f) => Boolean(fileContents[f])
    );
    const nestedReadmes = Object.entries(fileContents)
      .filter(
        ([filePath]) =>
          /^(?:apps|packages)\/[^/]+\/README(?:\.[^./]+)?\.md$/i.test(
            filePath
    ...
    const summary: ProjectSummary = {
      ...
      existingReadme: existingReadmePath
        ? {
            ...
            content: String(fileContents[existingReadmePath] || "").slice(
              0,
              20000
            ),
          }
  ...
  private detectLanguage(files: string[]): string {
    const counts: Record<string, number> = {
      TypeScript: files.filter((f) => f.endsWith(".ts") || f.endsWith(".tsx"))
        .length,
      JavaScript: files.filter((f) => f.endsWith(".js") || f.endsWith(".jsx"))
        .length,
