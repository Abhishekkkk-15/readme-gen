...
export async function generateReadmeSemanticCommand(options: {
  ...
  const provider =
    options.provider || (cfgProvider === "gemini" ? "gemini" : "groq");
  ...
  if (!apiKey) {
    ...
    console.log(
      chalk.yellow(
        "- GOOGLE_GENERATIVE_AI_API_KEY or readmegen config geminiKey (Gemini)"
      )
    );
    return;
  }
  ...
  const spinner = ora(
    "🔍 Analyzing local codebase (semantic pipeline)..."
  ).start();
  try {
    ...
    for (const r of quality.reasons.slice(0, 8))
